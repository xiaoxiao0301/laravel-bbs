    ...
    <div class="card mt-4">
        <div class="card-body active-users pt-2">
            ...
            @foreach ($activeUsers as $user)
                <a class="media mt-2" href="{{ route('users.show', $user->id) }}">
                    <div class="media-left media-middle mr-2 ml-1">
                        <img src="{{ $user->avatar }}" width="24px" height="24px" class="media-object">
                    </div>
    ...
    <div class="card mt-4">
        <div class="card-body pt-2">
            ...
            @foreach($links as $link)
                <a href="{{ $link->link }}" class="media mt-1" target="_blank">
                    <div class="media-body"><span class="media-heading text-muted">{{ $link->title }}</span></div>
                </a>
            @endforeach
        </div>
    </div>
